...
export default async function HomePage() {
  ...
  const now = new Date()
  // DRAFT = hidden (not in query)
  // PUBLISHED = upcoming (visible)
  // ACTIVE = traveling now (highlighted)
  // COMPLETED = history (grayscale)
  // CANCELLED = history (red)
            ...
            {active.length > 0 && (
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {active.map((tour) => (
                    <TourCard key={tour.id} tour={tour} variant="active" />
                  ))}
                </div>
              </div>
            )}

            {/* ── ทริปที่จะมาถึง (PUBLISHED) ── */}
            {upcoming.length > 0 && (
              <div>
                <div className="flex items-center gap-3 mb-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {upcoming.map((tour) => (
                    <TourCard key={tour.id} tour={tour} variant="upcoming" />
                  ))}
                </div>
              </div>
            )}

            {/* ── ประวัติการเดินทาง (COMPLETED + CANCELLED) ── */}
            {history.length > 0 && (
              <div>
                <div className="flex items-center gap-3 mb-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {history.map((tour) => (
                    <TourCard key={tour.id} tour={tour} variant={tour.status === 'CANCELLED' ? 'cancelled' : 'completed'} />
                  ))}
                </div>
              </div>
            )}
...
type TourItem = {
  ...
  primaryCountry: string; cities: string[]; startDate: Date | string; endDate: Date | string
  isChina: boolean; status: string; days: { id: string }[]
}
  ...
  return (
    <Link href={`/tour/${tour.id}/today`} className="flex group">
      <div className={`bg-white/80 backdrop-blur-xl rounded-2xl border overflow-hidden flex flex-col w-full
        ...
        {/* Cover */}
        <div className={`aspect-[16/9] relative flex-shrink-0 overflow-hidden ${
          isHistory ? 'bg-gray-100' : 'bg-gradient-to-br from-indigo-100 to-violet-100'
        }`}>
          {tour.coverImageUrl && (
            <Image
              src={tour.coverImageUrl} alt="" fill unoptimized
              className={`object-cover absolute inset-0 transition-all duration-500 group-hover:scale-105 ${
                isHistory ? 'grayscale group-hover:grayscale-0' : ''
              }`}
            />
          )}
          ...
          {variant === 'cancelled' && (
            <span className="absolute top-3 left-3 text-[10px] px-2.5 py-1 rounded-full bg-red-500 text-white font-bold">
              ยกเลิก
            </span>
          )}
          {variant === 'completed' && (
            <span className="absolute top-3 left-3 text-[10px] px-2.5 py-1 rounded-full bg-gray-500 text-white font-bold">
              เสร็จสิ้น
            </span>
          )}
          {tour.isChina && (
        ...
        <div className="p-4 flex flex-col flex-1">
          ...
          <div className="mt-3 pt-3 border-t border-gray-100/60 flex items-center justify-between gap-2">
            ...
            <span className={`text-[11px] font-semibold shrink-0 ${
              variant === 'active' ? 'text-emerald-600'
                : variant === 'cancelled' ? 'text-red-400'
                : variant === 'completed' ? 'text-gray-400'
                : 'text-indigo-600'
            }`}>
              {variant === 'cancelled' ? 'ยกเลิกแล้ว' : countdown.label}
            </span>
          </div>
        </div>
      </div>
