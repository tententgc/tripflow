            ...
            <Link key={day.id} href={`/tour/${tourId}/day/${day.dayNumber}`} className="block">
              <div className={`bg-white/80 backdrop-blur-xl rounded-2xl ${c.border} border p-4 active:scale-[0.99] transition-all relative overflow-hidden`}>
                ...
                <div className="flex items-center gap-1.5 mt-3 pl-2 flex-wrap">
                  ...
                  {day.mealBreakfast && <span className="text-[11px] px-2 py-1 rounded-lg font-medium border bg-orange-50/80 text-orange-600 border-orange-100/50">เช้า</span>}
                  {day.mealLunch && <span className="text-[11px] px-2 py-1 rounded-lg font-medium border bg-green-50/80 text-green-600 border-green-100/50">กลางวัน</span>}
                  {day.mealDinner && <span className="text-[11px] px-2 py-1 rounded-lg font-medium border bg-violet-50/80 text-violet-600 border-violet-100/50">เย็น</span>}
                </div>
                )}
