...
const typeConfig: Record<string, { label: string; emoji: string; gradient: string; textColor: string; badgeColor: string }> = {
  ...
  HOTEL_VOUCHER:    { label: 'เวาเชอร์โรงแรม',    emoji: '🏨', gradient: 'from-purple-600 to-purple-800', textColor: 'text-white', badgeColor: 'bg-purple-500' },
  ...
  PASSPORT:         { label: 'พาสปอร์ต',            emoji: '📘', gradient: 'from-indigo-600 to-indigo-800', textColor: 'text-white', badgeColor: 'bg-indigo-500' },
      ...
      <div className={`bg-gradient-to-br ${cfg.gradient} p-6 pb-8`}>
        <div className="flex items-start justify-between mb-4">
          <div>
            ...
            {doc.titleEn && (
              <p className={`text-sm opacity-70 mt-0.5 ${cfg.textColor}`}>{doc.titleEn}</p>
            )}
          </div>
        ...
        {doc.description && (
          <p className={`text-sm opacity-80 ${cfg.textColor}`}>{doc.description}</p>
        )}
      </div>
      ...
      <div className="bg-white p-6 flex flex-col items-center">
        {doc.qrData ? (
          ...
          <a
            ...
            className="flex flex-col items-center gap-2 py-8 text-blue-600"
          >
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="bg-white border-b border-gray-200 px-4 pt-12 pb-4 flex items-center gap-3">
        ...
        <div>
          <h1 className="font-bold text-gray-900">ตั๋วเข้าสถานที่</h1>
          <p className="text-xs text-gray-500">{tour.title}</p>
        </div>
        {docs.length > 0 && (
      ...
      {docs.length === 0 ? (
        ...
        <>
            ...
            {docs.map((_, i) => (
              <button
                ...
                className={`rounded-full transition-all ${
                  i === current ? 'w-6 h-2 bg-blue-600' : 'w-2 h-2 bg-gray-300'
                }`}
              />
            ))}
          </div>
          ...
          {/* Swipe hint */}
          {docs.length > 1 && (
            <p className="text-center text-xs text-gray-400 mt-1">ปัดซ้าย-ขวาเพื่อดูตั๋วอื่น</p>
          )}
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium transition-all ${
                      i === current
                        ? `${cfg.badgeColor} text-white shadow-md scale-105`
