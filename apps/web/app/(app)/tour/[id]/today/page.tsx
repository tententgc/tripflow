...
const catC: Record<string, string> = { SIGHTSEEING:'#3b82f6', FOOD:'#f97316', TRANSPORT:'#6b7280', ACCOMMODATION:'#a855f7', SHOPPING:'#ec4899', TEMPLE:'#eab308', NATURE:'#22c55e', NIGHTLIFE:'#9333ea' }
const catBg: Record<string, string> = { SIGHTSEEING:'#eff6ff', FOOD:'#fff7ed', TRANSPORT:'#f3f4f6', ACCOMMODATION:'#faf5ff', SHOPPING:'#fdf2f8', TEMPLE:'#fefce8', NATURE:'#f0fdf4', NIGHTLIFE:'#faf5ff' }
...
function SectionTitle({ emoji, icon, children }: { emoji?: string; icon?: React.ReactNode; children: React.ReactNode }) {
  return (
    <div className="flex items-center gap-2 mb-4">
      ...
      <h2 className="text-[15px] font-bold" style={{ color: '#3d2c1e' }}>{children}</h2>
    </div>
  )
}
...
export default function TodayPage() {
  ...
  if (isLoading) return (
    <div className="min-h-screen animate-pulse" style={{ background: '#f3f4f6' }}>
      <div className="px-5 pt-20 space-y-4 max-w-6xl mx-auto">
        <div className="h-40 bg-white rounded-2xl" />
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
          <div className="lg:col-span-7 space-y-4"><div className="h-48 bg-white rounded-2xl" /><div className="h-32 bg-white rounded-2xl" /></div>
          <div className="lg:col-span-5 space-y-4"><div className="h-56 bg-white rounded-2xl" /><div className="h-40 bg-white rounded-2xl" /></div>
        </div>
      </div>
    </div>
  )
  if (!tour) return <div className="min-h-screen flex items-center justify-center" style={{ background: '#faf8f5' }}><p className="text-gray-500">ไม่พบทริปนี้</p></div>
  ...
  if (!cd) return <div className="min-h-screen pb-24" style={{ background: '#faf8f5' }}><TopBar title={tour.title} subtitle="ยังไม่มีกำหนดการ" backHref="/home" /><BottomNav activeTab="today" tourId={tourId} isChina={tour.isChina} /></div>
  ...
  return (
    <Bubble>
      <div className="p-6 relative overflow-hidden">
        ...
        <div className="absolute -top-10 -right-10 w-36 h-36 rounded-full opacity-[0.07]" style={{ background: 'radial-gradient(circle, #f97316, transparent)' }} />
        ...
        <div className="relative">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[12px] text-orange-600/70 font-semibold">{td} วัน {tn} คืน · {tour.members.length} คน</p>
            </div>
            <Tag emoji="✨" bg="#fff7ed" color="#c2410c">เตรียมตัว</Tag>
          </div>

          <div className="flex items-end gap-3 mt-2">
            <div>
              <p className="text-[12px] font-bold text-orange-400 uppercase tracking-wider mb-1">ออกเดินทางอีก</p>
              <div className="flex items-end gap-1.5">
                <span className="text-[72px] font-black leading-none tabular-nums"
                  style={{ color: '#ea580c', letterSpacing: '-0.04em' }}>
                  {av}
                </span>
                <span className="text-[22px] font-bold text-orange-300 pb-2">วัน</span>
              </div>
            </div>
            <div className="flex-1 pb-3">
              <div className="text-right text-[12px] font-medium text-orange-400 mb-1">
...
function MealStrip({ b, l, d }: { b: boolean; l: boolean; d: boolean }) {
  const meals = [
    b && { emoji: '🌅', label: 'เช้า', accent: '#f97316' },
    l && { emoji: '🍱', label: 'กลางวัน', accent: '#22c55e' },
    d && { emoji: '🌙', label: 'เย็น', accent: '#f59e0b' },
...
/* ═══ Flight ═══ */
function FlightBubble({ flight: f }: { flight: Flight }) {
      ...
      {days.slice(0,3).map((w, i) => {
        ...
        return (
          <Bubble key={w.date} className="flex-1 text-center" color={i === 0 ? 'orange' : 'white'}>
            <div className="p-4">
              <p className="text-[12px] font-bold" style={{ color: i === 0 ? '#c2410c' : '#6b7280' }}>
...
function ContactsBubble({ contacts }: { contacts: TourData['contacts'] }) {
  ...
  const typeIcon: Record<string, { icon: React.ReactNode; bg: string; color: string }> = {
    THAI_GUIDE:   { icon: <User size={18} />,               bg: '#fff7ed', color: '#ea580c' },
    ...
    RESTAURANT:   { icon: <UtensilsCrossed size={18} />,     bg: '#fff7ed', color: '#ea580c' },
        ...
        <div className="divide-y divide-gray-100/80">
          {tour.days.map(day => (
            <a key={day.id} href={`/tour/${tourId}/day/${day.dayNumber}`}
              className="flex items-center gap-3.5 px-5 py-4 hover:bg-orange-50/40 transition-colors no-btn-fx opacity-100 hover:opacity-100">
              <div className="w-12 h-12 rounded-2xl flex items-center justify-center text-[13px] font-black"
                style={{ background: 'linear-gradient(135deg, #fff7ed, #fed7aa)', color: '#c2410c' }}>
                D{day.dayNumber}
              </div>
              <div className="flex-1 min-w-0">
    ...
    <>
      ...
      <Bubble>
        ...
        <div className="p-5 flex items-center justify-between" style={{ borderBottom: '1px solid rgba(0,0,0,0.04)' }}>
          ...
          <div className="flex gap-1.5">
            {cd.mealBreakfast && <span className="w-9 h-9 rounded-xl flex items-center justify-center text-sm" style={{ background: '#fff7ed' }}>🌅</span>}
        ...
        <div className="p-5 pt-0">
          <a href={`/tour/${tourId}/day/${cd.dayNumber}`}
            ...
            style={{ background: 'linear-gradient(135deg, #f97316, #ea580c)', color: '#fff', boxShadow: '0 4px 14px rgba(249,115,22,0.25)' }}>
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mt-3">
        ...
        <button onClick={() => setAll(true)} className="text-[12px] font-bold text-orange-600 hover:text-orange-800 no-btn-fx flex items-center gap-1">
  ...
  return (
    <Bubble color={a.isPinned ? 'amber' : 'white'}>
      <div className="p-5">
        ...
        {(cl || exp) && <button onClick={() => setExp(v => !v)} className="text-[12px] font-bold text-orange-600 mt-2 no-btn-fx">{exp ? 'ย่อ' : 'ดูเพิ่มเติม'}</button>}
...
function AnnModal({ anns, onClose, onImg }: { anns: Announcement[]; onClose: () => void; onImg: (s: string) => void }) {
  ...
  return (
    ...
    <div onClick={e => e.stopPropagation()} style={{ position:'fixed', zIndex:201, top:'50%', left:'50%', width:'92vw', maxWidth:'480px', maxHeight:'75vh', borderRadius:'24px', overflow:'hidden', background:'#fff', boxShadow:'0 32px 80px rgba(0,0,0,0.2)', display:'flex', flexDirection:'column' as const, animation:`${closing?'mdPopOut':'mdPop'} .3s cubic-bezier(.34,1.56,.64,1) both` }}>
      <div className="flex-shrink-0 px-6 py-5 flex items-center justify-between" style={{ borderBottom: '1px solid rgba(0,0,0,0.06)', background: '#fff7ed' }}>
        <div className="flex items-center gap-3">
          <span className="text-2xl">📢</span>
          <div>
            <h2 className="text-[16px] font-bold text-gray-800">ประกาศทั้งหมด</h2>
            <p className="text-[12px] text-orange-600 font-bold mt-0.5">{anns.length} ประกาศ</p>
          </div>
        </div>
